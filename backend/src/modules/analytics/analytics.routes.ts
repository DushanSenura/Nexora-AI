...
async function devStats() {
  const users = await readJson<Array<{ disabled_at?: string | null }>>("dev-users.json", []);
  const chatStore = await readJson<{ chats?: unknown[]; messages?: unknown[] }>("dev-chats.json", {});
  const documentStore = await readJson<{ documents?: Array<{ status?: string }> }>("dev-documents.json", {});
  const agentTasks = await readJson<Array<{ agent_type: string }>>("dev-agent-tasks.json", []);
