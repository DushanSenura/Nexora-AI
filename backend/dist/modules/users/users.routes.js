...
import { canUseDevAuthFallback, deleteDevUser, findDevUserById, listDevUsers, setDevUserDisabled, toPublicUser, updateDevUserPassword, updateDevUserProfile, } from "../auth/devAuthStore.js";
...
usersRouter.get("/", requireAuth, requireAdmin, async (req, res, next) => {
    try {
        ...
        const result = await query("select id, name, email, role, disabled_at, created_at from users where ($1 = '' or name ilike $2 or email ilike $2) order by created_at desc", [search, `%${search}%`]);
...
usersRouter.patch("/:userId/disabled", requireAuth, requireAdmin, async (req, res, next) => {
    ...
    try {
        ...
        const result = await query("update users set disabled_at = case when $1 then now() else null end where id = $2 returning id, name, email, role, disabled_at, created_at", [disabled, userId]);
        if (!result.rows[0]) {
            res.status(404).json({ message: "User not found" });
            return;
        }
