...
function endpointForAgent(agentType) {
...
agentsRouter.get("/", async (req, res, next) => {
    try {
        ...
        const result = await query(`select * from agent_tasks where user_id = $1 ${filter} order by created_at desc`, params);
...
agentsRouter.post("/", async (req, res, next) => {
    try {
        ...
        const result = await query("insert into agent_tasks (user_id, agent_type, input, output, status) values ($1, $2, $3, $4, 'complete') returning *", [req.user.id, input.agentType, input.input, aiResponse.data.output]);
